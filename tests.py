"""Comprehensive correctness tests for tokenizer, model, and compression analysis."""
import sys, os
sys.path.insert(0, '.')
...
def test_model():
    ...
    x2 = x1.clone()
    x2[0, 32] = (x2[0, 32] + 1) % cfg.vocab_size
    model.eval()
    with torch.no_grad():
        l1, _ = model(x1)
        l2, _ = model(x2)
